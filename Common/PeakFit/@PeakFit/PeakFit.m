...
%                curve in order to filter noise before finding the maxima. This
...
%
%  Weights: Weights for the fit, specified as the comma-separated pair 
%           consisting of 'Weights' and a vector the same size as the 
%           response data Y data points of the curve. The default is to use
%           no weights.
%
...
% Weights functionality added by Evert Merkx
...
methods
    ...
    function x=get.Weights(obj)
		if numel(obj.YData)==numel(obj.Weights)
			x=[obj.Weights];
		else
			x=[];
		end
    ...
    function obj=set.Weights(obj,x)
		if isempty(x)
			obj.Weights=[];
		elseif isrealvector(x) && all(isfinite(x))
			if numel(x)<obj.MinNumPoints
				error('PeakFit:setWeights:InsufficientNumPoints',...
					'Input vector to the Weigths must have at least %d elements.',...
					obj.MinNumPoints);
			else
				obj.Weights=x(:).';
			end
		else
			error('PeakFit:setWeights:InvalidInput',...
				'Input to set the Weights must be a vector of finite real numbers.');
		end
	end

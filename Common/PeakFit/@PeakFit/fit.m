...
assert(numel(wt)==numel(y) || isempty(wt),...
	'PeakFit:fit:InconsistentNumPoints',...
	'The number of Y data points and Weights must be equal, or Weights should be empty.');

% Sort the points (x,y,w) in ascending order
[x,ix]=sort(x);
y=y(ix);
if ~isempty(wt)
    wt=wt(ix);

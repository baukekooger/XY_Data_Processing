...
t_start_index = find(obj.digitizer.time == t_start); 
t_stop = obj.datapicker.StopTimeSpinner.Value;
t_stop_index = find(obj.digitizer.time == t_stop); 

% select the time range  
obj.plotdata.spectra_decay = obj.digitizer.spectra(:, :, ex_index, ...
    t_start_index:t_stop_index);
obj.plotdata.time_decay = obj.digitizer.time(t_start_index:t_stop_index);
...
if mean_beginning < 0.5*mean_maxvalues
    obj.plotdata.spectra_decay = obj.digitizer.spectra(:, :, ex_index, ...
    t_start_index:t_stop_index) ./ max(obj.plotdata.spectra_decay, [], 4); 
else
    obj.plotdata.spectra_decay = obj.digitizer.spectra(:, :, ex_index, ...
    t_start_index:t_stop_index) ./ mean_maxvalues; 
end
